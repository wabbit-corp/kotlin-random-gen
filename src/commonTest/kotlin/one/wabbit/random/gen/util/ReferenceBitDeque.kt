...
/**
 * A reference bit deque storing each bit as Boolean in an ArrayDeque. We use it to validate the
 * behavior of MutableBitDeque.
 */
class ReferenceBitDeque {
    ...
    /**
     * fillAndSet(index, value): if index < size, set existing; else append false bits until index,
     * then set index-th bit.
     */
    fun fillAndSet(index: Long, value: Boolean) {
        ...
        repeat((needed - 1).toInt()) { buf.add(false) }
        buf.add(value)
    }

    /**
     * Add 8 bits from a Byte, in MSB_FIRST or LSB_FIRST order. Must cast to Int for the mask to
     * avoid sign issues.
     */
    ...
    fun removeFirst(): Boolean = buf.removeFirst()

    /**
     * Remove the first n bits in the given order, returning them as a Long. If not enough bits
     * remain, this should throw NoSuchElementException.
     */
