...
sealed interface Gen<out A> {
    ...
    fun repeat(count: Int): Gen<List<A>> =
        ListOf(Done(count), this)
    fun repeat(count: Gen<Int>): Gen<List<A>> =
        ListOf(count, this)

    infix fun <B> zipRight(that: Gen<B>): Gen<B> =
        FlatMap(this) { that }

    infix fun <B> zipLeft(that: Gen<B>): Gen<A> =
    ...
    fun nullable(): Gen<A?> =
        Choose(Wheel.unweighted(listOf(Done(null), this)))

    fun label(label: String): Gen<A> =
        Label(label, this)
    ...
    companion object {
        ...
        fun <A> label(label: String, body: Gen<A>): Gen<A> =
            Label(label, body)
        fun <A> label(label: String, body: () -> Gen<A>): Gen<A> =
            Label(label, delay(body))

        // Side-effecting combinator
        fun <A> Gen<A>.tap(side: (A) -> Unit): Gen<A> =
            flatMap { a -> side(a); Gen.const(a) }
        ...
        // Zip combinators
        fun <A, B> zip(a: Gen<A>, b: Gen<B>): Gen<Pair<A, B>> =
            a zip b
        ...
        fun <A, B, C, D, Z> map(a: Gen<A>, b: Gen<B>, c: Gen<C>, d: Gen<D>, f: (A, B, C, D) -> Z): Gen<Z> =
            zip(a, b, zip(c, d)).map { (a, b, cd) -> val (c, d) = cd; f(a, b, c, d) }
        ...
        fun <A, B, C, D, Z> flatMap(a: Gen<A>, b: Gen<B>, c: Gen<C>, d: Gen<D>, f: (A, B, C, D) -> Gen<Z>): Gen<Z> =
            zip(a, b, zip(c, d)).flatMap { (a, b, cd) -> val (c, d) = cd; f(a, b, c, d) }
        ...
        fun <T> listOf(length: Gen<Int>, element: Gen<T>): Gen<List<T>> =
            ListOf(length, element)
        fun <T> listOf(range: IntRange, element: Gen<T>): Gen<List<T>> =
            ListOf(int(range), element)
        fun <R> listOf(count: Int, gen: Gen<R>): Gen<List<R>> =
            ListOf(Done(count), gen)
        ...
        val string get() = string(0..8)

        // Functions
        fun <A, Z> func(
            returning: Gen<Z>, a: CoGen<A> = CoGen.fromBuiltinHashCode(),
            buckets: Gen<Int> = int(1..8)
        ): Gen<(A) -> Z> =
            bits(64).flatMap { salt ->
        ...
        private val validUnicodeCodePoint: Gen<Int> = oneOfGen(
            int(0x0000..0xD7FF),
            int(0xE000..0xFFFF),
            int(0x10000..0x10FFFF)
        )
        ...
        fun <T> tree(constructors: List<(Gen<T>) -> Gen<T>>): Gen<T> =
            recursive { self ->
